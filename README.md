...
In this guide we we''ll need a Kubernetes cluster for testing. Let's create one using [Minikube](https://minikube.sigs.k8s.io/docs/tutorials/kubernetes_101/module1/) </br>
...
### run Alpine Linux: 
```
docker run -it --rm -v ${HOME}:/root/ -v ${PWD}:/work -w /work --net host -e KUBECONFIG=/root/.kube/config alpine sh
```
...
The controller ships as a `helm` chart, so we can grab version `v1.12.2` as per the compatibility
matrix. </br>
...
mkdir ./nginx-ingress-controller/manifests/

helm template ingress-nginx ingress-nginx \
--repo https://kubernetes.github.io/ingress-nginx \
--version ${CHART_VERSION} \
--namespace ingress-nginx \
> ./nginx-ingress-controller/manifests/nginx-ingress.${APP_VERSION}.yaml
```
...
kubectl apply -f ./nginx-ingress-controller/manifests/nginx-ingress.${APP_VERSION}.yaml
```
...
Note that we dont have load balancer capability in `minikube` by default, so our `LoadBalancer` is pending:
...
```
kubectl apply -f ./nginx-ingress-controller/features/service-a.yaml
kubectl apply -f ./nginx-ingress-controller/features/service-b.yaml
```
